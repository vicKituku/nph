...
function StatsSection({ stats, position }: { stats: any; position: string }) {
  return (
    <div className="space-y-6">
      <h4 className="heading-3">Statistics</h4>
      ...
      <div className="grid grid-cols-3 gap-4">
        <StatCard
          ...
          subtitle={`${stats.matches.started} starts`}
          icon={
        ...
        <StatCard
          ...
          subtitle={`${stats.goals.assists} assists`}
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {position === 'Forward' && (
        <div className="grid grid-cols-2 gap-4">
          <StatCard
            ...
            value={stats.goals.shotsOnTarget}
            subtitle={`${stats.goals.shots} total shots`}
          />
          <StatCard
            title="Conversion Rate"
            value={stats.goals.conversionRate}
      ...
      <div className="grid grid-cols-2 gap-4">
        <StatCard
          title="Pass Accuracy"
          value={stats.passing.accuracy}
          subtitle={`${stats.passing.keyPasses} key passes`}
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <StatCard
          ...
          value={stats.dribbling.successRate}
          subtitle={`${stats.dribbling.successful}/${stats.dribbling.attempts} successful`}
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Defensive Stats (for Midfielders and Defenders) */}
      {(position === 'Midfielder' || position === 'Defender') && (
        <div className="grid grid-cols-3 gap-4">
          <StatCard
            title="Tackles"
            value={stats.defensive.tackles}
            subtitle="Successful tackles"
            icon={
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <StatCard
            title="Interceptions"
            value={stats.defensive.interceptions}
            subtitle="Ball recoveries"
          />
          <StatCard
            title="Clearances"
            value={stats.defensive.clearances}
            subtitle="Defensive actions"
          />
          {position === 'Defender' && stats.defensive.blocks && (
            <>
              <StatCard
                title="Blocks"
                value={stats.defensive.blocks}
                subtitle="Shot blocks"
              />
              <StatCard
                title="Duels Won"
                value={stats.defensive.duelsWon}
